...
void MyVeinsNodeApp::initialize(int stage) {
	try {
		...
		if (stage == 0) {
			...
			tempWsmCount = 0;
			mvec.setName(std::string("monitor:").append(std::to_string(id2num(myId,node0id))).c_str());
...
void MyVeinsNodeApp::onWSM(BaseFrame1609_4 *frame) { //TODO restructure to remove redundant code
	...
	recordScalar(std::string("startedWSM-").append(std::to_string(++tempWsmCount)).c_str(), tempWsmCount);
...
recordScalar(std::string("endedWSM-").append(std::to_string(tempWsmCount)).c_str(), tempWsmCount);
}
...
recordScalar(std::string("startedSelfMessage-").append(std::to_string(simTime().raw())).c_str(), tempWsmCount);
try {
...
recordScalar(std::string("endedSelfMessage-").append(std::to_string(simTime().raw())).c_str(), tempWsmCount);
}
